            ...
            <div class="col-md-12">
                <div class="card">
                    ...
                    <div class="card-content">
                        ...
                        <h4>Asociaci&oacute;n de Temas, <b> Grupo de Transformaci&oacute;n <?= $gt->id ?></b></h4>                        
                            ...
                            <table id="datatables" class="table table-striped table-no-bordered table-hover" cellspacing="0" width="100%" style="width:100%">
                                ...
                                <tbody>
                                    ...
                                    foreach ($siGtTemas as $siGtTema) {
                                        ...
                                        <tr>
                                            <td><?= $siGtTema->id ?></td>
                                            <td><?= $siGtTema->si_tema->tema ?></td>
                                            <td><?= ($siGtTema->fecha) ? date_format($siGtTema->fecha, 'Y-m-d') : '' ?></td>
                                           
                                            <td>
                                                <?php
                                                if ($siGtTema->status_id == 1) {
                                                    echo '<span class="label label-success">' . $siGtTema->ma_status->value . '</span>';
                                                } elseif ($siGtTema->status_id == 2) {
                                                    echo '<span class="label label-danger">' . $siGtTema->ma_status->value . '</span>';
                                                } elseif ($siGtTema->status_id == 3) {
                                                    echo '<span class="label label-danger">' . $siGtTema->ma_status->value . '</span>';
                                                } else {
                                            ...
                                            <td class="text-right">
                                                ...
                                                <?=
                                                $this->Html->link($this->Html->image('/img/ico_editar.png', ['style' => 'width: 20px; height: auto; margin-left: 10px', "alt" => __('Editar'), "title" => __('Editar')]), ['action' => 'edit2', $siGtTema->id, $id], ['escape' => false]);
                                                ?>

                                                <?php
                                                echo $this->Form->postLink(
                                                        $this->Html->image('/img/ico_delete.jpg', ['style' => 'width: 20px; height: auto; margin-left: 10px', "alt" => __('Eliminar'), "title" => __('Eliminar')]), ['action' => 'delete2', $siGtTema->id, $id], ['escape' => false, 'confirm' => __('¿Está seguro?')]
                                                );
                                                ?>
                                            </td>
