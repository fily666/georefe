            ...
            <div class="col-md-12">
                <div class="card">
                    ...
                    <ol class="breadcrumb pull-right">
                        ...
                        <li class="active">Asistentes &Eacute;xodo</li>
                    </ol>

                    <div class="card-content">

                        <h4>Asistentes &Eacute;xodo, <b><?= $exodo->descripcion ?></b></h4>                        
                                ...
                                <tbody>
                                    <?php foreach ($siExodoAsistentes as $siExodoAsistente) { ?>
                                        <tr>
                                            ...
                                            <td><?= $siExodoAsistente->ma_propiedade->valor ?></td>
                                            <td><?= $siExodoAsistente->persona->telefono1 ?></td>
                                            <td><?= $siExodoAsistente->persona->celular ?></td>
                                            <td><?= $siExodoAsistente->guium->nombres . ' ' . $siExodoAsistente->guium->apellidos ?></td>
                                            <td><?= $siExodoAsistente->si_pastore->person->nombres . ' ' . $siExodoAsistente->si_pastore->person->apellidos ?></td>

                                            <td>
                                                <?php
                                                if ($siExodoAsistente->status_id == 1) {
                                                    echo '<span class="label label-success">' . $siExodoAsistente->ma_status->value . '</span>';
                                                } elseif ($siExodoAsistente->status_id == 2) {
                                                    echo '<span class="label label-danger">' . $siExodoAsistente->ma_status->value . '</span>';
                                                } elseif ($siExodoAsistente->status_id == 3) {
                                                    echo '<span class="label label-danger">' . $siExodoAsistente->ma_status->value . '</span>';
                                                } else {
                                            ...
                                            <td class="text-right">
                                                ...
                                                echo $this->Form->postLink(
                                                        $this->Html->image('/img/ico_delete.jpg', ['style' => 'width: 20px; height: auto; margin-left: 10px', "alt" => __('Eliminar'), "title" => __('Eliminar')]), ['action' => 'delete3', $siExodoAsistente->id, $exodo->id], ['escape' => false, 'confirm' => __('¿Está seguro?')]
                                                );
                                                ?>
                                            </td>  
                                        </tr>
